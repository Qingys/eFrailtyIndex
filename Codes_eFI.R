...
########import data--------
dt_997 <- read_sav('997FI病历资料死亡数据-病案号登记号和姓名合并.sav')
dt_1098 <- read_sav('1098衰弱指数和肺癌化疗-全.sav')
dt_1020 <- read_sav('1020FI-lab短期结局指标数据.sav')
###tidy
dt_all <- dt_1098 %>% 
  select(
    ###baseline
    ...
    体重,身高,嗜酒,吸烟指数,吸烟状态,
    ###short-term outcome
    感染,消化道反应,皮疹,发热,化疗中断,肝功能受损,骨髓抑制总,住院天数,
    ###covariates
    细胞类型_A,方案,有无转移,分期,放疗,手术人次数,
    ###FI lab
...
dt_FIlab <- dt_1020 %>% 
  select(
    ###FI lab
    实验室FI,实验室frailty,病案号)
dt_death <- dt_997 %>% 
  select(病案号,deathdatanew,deathnew,确诊日期) %>% 
  mutate(deathdatanew=case_when(病案号=='IP0001152258' ~ '2017-09-25',
                                病案号=='IP0000558567' ~ '2017-02-01',
                                   TRUE ~ as.character(deathdatanew))) %>% 
  mutate(确诊日期=as.Date(确诊日期), deathdatanew=as.Date(deathdatanew)) %>% 
  mutate(surv_time=difftime(deathdatanew, 确诊日期, units='days')/30)
###bind data
dt_bind <- left_join(dt_all, dt_FIlab, by='病案号')
dt_bind <- left_join(dt_bind, dt_death, by='病案号')
dt <- dt_bind %>% filter(!is.na(surv_time))  ###997
